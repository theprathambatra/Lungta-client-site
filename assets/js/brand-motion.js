(() => {
  ...
  style.textContent = `
    .lungta-cape-live{transform-origin:88% 50%;will-change:transform,filter;animation:lungtaCapeFlight 5.6s cubic-bezier(.37,0,.2,1) infinite;filter:url(#lungtaGlobalCapeWave) drop-shadow(0 18px 40px rgba(0,0,0,.24))}
    .lungta-cape-live:hover{animation-duration:3.35s}
    @keyframes lungtaCapeFlight{
      0%,100%{transform:translate3d(0,0,0) scaleX(1) scaleY(1)}
      18%{transform:translate3d(-4px,-5px,0) scaleX(1.008) scaleY(.996)}
      38%{transform:translate3d(2px,2px,0) scaleX(.997) scaleY(1.004)}
      58%{transform:translate3d(-3px,-7px,0) scaleX(1.011) scaleY(.994)}
      78%{transform:translate3d(1px,-2px,0) scaleX(1.002) scaleY(1.001)}
    }
  ...
  svg.innerHTML = `<defs><filter id="lungtaGlobalCapeWave" x="-18%" y="-28%" width="136%" height="156%"><feTurbulence id="lungtaCapeNoise" type="fractalNoise" baseFrequency="0.004 0.012" numOctaves="2" seed="11" result="noise"/><feDisplacementMap id="lungtaCapeDisplace" in="SourceGraphic" in2="noise" scale="4.2" xChannelSelector="R" yChannelSelector="G"/></filter></defs>`;
      ...
      const run = () => {
        try {
          ...
          for (let i=0;i<d.length;i+=4) {
            ...
            if (m<=2) { d[i]=d[i+1]=d[i+2]=d[i+3]=0; continue; }
            const a=m/255;
          ...
          console.warn('LUNGTA brand matte cleanup fallback',err);
        }
        resolve();
      };
  ...
  if (!reduced && capes.length && noise && displace) {
    const tick = now => {
      ...
      displace.setAttribute('scale',(capeHover ? 8.2+Math.sin(phase)*1.4 : 4.2+Math.sin(phase*.78)*.8).toFixed(2));
